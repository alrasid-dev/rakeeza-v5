...
export async function GET(req: NextRequest) {
  ...
  children.push(
    new Paragraph({
      ...
      spacing: { after: 200 },
      border: {
        bottom: { style: BorderStyle.SINGLE, size: 18, color: GOLD, space: 6 },
      },
      children: [
        new TextRun({
  ...
  // Parties table if present (structure columns like الصك — empty-friendly)
  ...
  if (doc.qrPayload || doc.number) {
    ...
    children.push(
      new Paragraph({
        ...
        children: [
          new TextRun({
            text: `رمز التحقق / QR: ${doc.number || doc.id}`,
            size: 16,
            color: GREEN,
            font: 'Arial',
            rightToLeft: true,
